//{ Driver Code Starts
// Initial template for C++

#include <bits/stdc++.h>
using namespace std;


// } Driver Code Ends
// User function template for C++

class Solution{
    public:
    bool isSafe(int x, int y, vector<vector<int>> &mat, vector<vector<int>>& visited, int n){
        if(x>=0 && x<n && y>=0 && y<n && mat[x][y]==1 && visited[x][y]==0)
            return true;
        return false;
    }
    
    void solve(vector<vector<int>> &mat, int x, int y, vector<string>& ans, string path, 
                vector<vector<int>>& visited, int n){
        if(x==n-1 && y==n-1){
            ans.push_back(path);
            return;
        }
        visited[x][y]=1;
    
        int di[4]={1, 0, 0, -1};
        int dj[4]={0, -1, 1, 0};
        string dir="DLRU";
    
        for(int ind=0; ind<4; ind++){
            int newx=x+di[ind];
            int newy=y+dj[ind];
    
            if(isSafe(newx, newy, mat, visited, n)){
                solve(mat, newx, newy, ans, path+dir[ind], visited, n);
            }
        }
    
        visited[x][y]=0;
    }
    
    vector<string> findPath(vector<vector<int>> &m, int n) {
        vector<vector<int>> visited(n, vector<int> (n, 0));
        vector<string> ans;
        string path="";
        
        if(m[0][0] == 0)
            return {"-1"};
        
        solve(m, 0, 0, ans, path, visited, n);
        sort(ans.begin(), ans.end());
        return ans;
    }
};

    


//{ Driver Code Starts.

int main() {
    int t;
    cin >> t;
    while (t--) {
        int n;
        cin >> n;
        vector<vector<int>> m(n, vector<int> (n,0));
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                cin >> m[i][j];
            }
        }
        Solution obj;
        vector<string> result = obj.findPath(m, n);
        sort(result.begin(), result.end());
        if (result.size() == 0)
            cout << -1;
        else
            for (int i = 0; i < result.size(); i++) cout << result[i] << " ";
        cout << endl;
    }
    return 0;
}
// } Driver Code Ends